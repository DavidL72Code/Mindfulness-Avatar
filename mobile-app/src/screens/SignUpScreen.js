...
import { LanguageChips, LanguageMenuButton } from '../components/LanguageSelector';
import {
...
export default function SignUpScreen({ navigation }) {
  ...
  const copy = LOCALIZED_CONTENT[languagePreference]?.ui || LOCALIZED_CONTENT.en.ui;

  useLayoutEffect(() => {
    const c = LOCALIZED_CONTENT[languagePreference]?.ui || LOCALIZED_CONTENT.en.ui;
  ...
  const handleSignUp = async () => {
    ...
    if (!trimmedFirstName || !trimmedLastName || !dob || !normalizedEmail || !password || !confirmPassword) {
      Alert.alert(copy.errorTitle, copy.errorFillAll);
      return;
    }
    if (password !== confirmPassword) {
      Alert.alert(copy.errorTitle, copy.passwordsDoNotMatch);
      return;
    }
    if (password.length < 8) {
      Alert.alert(copy.errorTitle, copy.passwordTooShort);
      return;
    }
    ...
    try {
      ...
      let errorMessage = copy.signUpGenericError;
    ...
    >
      <SafeAreaView style={styles.safe} edges={['top', 'bottom']}>
        ...
        <View style={styles.langCorner}>
          <LanguageMenuButton value={languagePreference} onChange={setLanguage} light />
        </View>
        ...
        <View style={styles.layout}>
          <View style={styles.content}>
            ...
            <LanguageChips value={languagePreference} onChange={setLanguage} />
...
const styles = StyleSheet.create({
  ...
  layout: { flexGrow: 1, maxWidth: 440, width: '100%', alignSelf: 'center' },
  content: {
    flex: 1,
